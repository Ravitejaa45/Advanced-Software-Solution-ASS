...
async function extractKeys() {
  ...
  status.textContent = `Successfully extracted ${
    (data.keys || []).length
  } keys`;
}
...
async function saveRule() {
  ...
  if (!res.ok) {
    ...
    status.textContent = "Successfully saved rule!";
    await loadRules();
  }
}
...
async function loadRules() {
  ...
  rules.forEach((r) => {
    ...
    info.innerHTML = `<div><b>${
      r.name
    }</b> — <span class="badge bg-secondary">${r.label}</span> <small>(p:${
      r.priority
    })</small> ${
      r.active ? "" : '<span class="badge bg-warning text-dark">disabled</span>'
    }</div>
                      <div class="text-muted">${
                        r.conditions.length
                      } condition(s), ${
      new Set(r.conditions.map((c) => c.group)).size
    } group(s)</div>`;
    ...
    toggle.onclick = async () => {
      await fetch(`${API_BASE}/rules/${r.id}/toggle`, {
        method: "POST",
        headers: USER_HEADER,
      });
      loadRules();
...
async function processNow() {
  ...
  const data = await res.json();
  out.textContent = "Successfully processed:\n" + JSON.stringify(data, null, 2);
}
...
document.getElementById("btn-add-condition").onclick = () => {
  const keys = Array.from(
    document.getElementById("available-keys").options
  ).map((o) => o.value);
...
document.getElementById("btn-add-group").onclick = () => {
  ...
  let maxg = 0;
  rows.forEach((r) => {
    maxg = Math.max(maxg, parseInt(r.dataset.group || "1", 10));
  });
